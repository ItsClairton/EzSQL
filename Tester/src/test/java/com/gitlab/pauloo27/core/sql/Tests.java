...
public class Tests {
    ...
    public void testBuilders() {
        ...
        // Test insert
        printAndAssert("INSERT INTO ${table} (name, phone, email) VALUES (?, ?, ?), (?, ?, ?);", new Insert("name, phone, email",
                new Insert.EzValue("' or 1=1", 666, "sql_injection@test.com"),
                new Insert.EzValue("People0", 30213, "people@sample.com")).toString());

        // Test select
        printAndAssert("SELECT name, phone, email FROM ${table} ORDER BY name ASC LIMIT 3;", new Select("name, phone, email")
                .orderBy("name", StatementBase.OrderByType.ASC)
                .limit(3).toString());

        // Test update
        printAndAssert("UPDATE ${table} SET name = ? WHERE name = ?;", (new Update().set("name", "People01").where().equals("name", "People1").toString()));

        // Test delete
        printAndAssert("DELETE FROM ${table} WHERE name = ? OR age > ?;", new Delete().where().equals("name", "People0").or().moreThan("age", 18).toString());
        ...
        printAndAssert(
                ...
                new Select("pedidos.id, clientes.nome, clientes.telefone, pedidos.criadoEm")
                        .join(
